...
- Columns: [X, Y, Z, Previous_Frame_Detected]
- 3D World Positions: Real 3D positions relative to hips (pose_world_landmarks)
- Previous_Frame_Detected: 0 if previous frame was NOT detected, 1 if it was detected
...
def extract_all_keypoints_from_video(
    ...
    # First pass: Count ALL frames to get exact count (don't trust CAP_PROP_FRAME_COUNT)
    ...
    # Initialize output array with EXACT frame count
    # Shape: [frame_idx, landmark_idx, (x, y, z, previous_frame_detected)]
    landmarks_array = np.zeros((actual_frame_count, 33, 4), dtype=np.float32)
    ...
    with tqdm(total=actual_frame_count, desc=f"  Processing {output_name}", leave=False) as pbar:
        while True:
            ...
            if results.pose_world_landmarks:
                # Extract all 33 landmarks with 3D world positions (relative to hips)
                # Set previous_frame_detected: 0 if previous was not detected, 1 if it was
                prev_detected_value = 1.0 if previous_frame_detected else 0.0
                
                for landmark_idx, landmark in enumerate(results.pose_world_landmarks.landmark):
                    landmarks_array[frame_idx, landmark_idx, 0] = landmark.x
                    landmarks_array[frame_idx, landmark_idx, 1] = landmark.y
                    landmarks_array[frame_idx, landmark_idx, 2] = landmark.z
                    landmarks_array[frame_idx, landmark_idx, 3] = prev_detected_value
                
                processed_count += 1
                previous_frame_detected = True  # Current frame detected, set for next frame
            else:
                # Frame was skipped - leave as zeros [0, 0, 0, 0]
    ...
    print(f"  ✅ Saved: {output_name} | Frames: {actual_frame_count} | Detected: {processed_count} ({success_rate:.1f}%)")
...
def batch_process_all_videos(
    ...
    # Only processing S4 and S5 (S1, S2, S3 already done on another computer)
    subjects = ['S4', 'S5']
    # Only processing the 5 specific actions available in download.txt
    actions = ['acting3', 'freestyle1', 'freestyle3', 'rom3', 'walking2']
    ...
    print(f"🎯 Format: (Frames, 33, 4) - Float32 - [X, Y, Z, Previous_Frame_Detected]")
    ...
    print(f"📊 Array format: (Frames, 33, 4) - Float32")
    print("="*80)
...
Examples:
  ...
  python extract_all_blazepose_keypoints.py --verify
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help='Number of parallel worker processes for videos (set 1 for sequential, default 4 to avoid memory overflow)'
    )
