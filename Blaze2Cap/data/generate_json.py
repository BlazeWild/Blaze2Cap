...
# 2. Define path relative to the script
# Point to final_numpy_dataset at project root
dataset_root = os.path.join(script_dir, "..", "..", "final_numpy_dataset")

# 3. Output file (saved in Blaze2Cap/dataset/Totalcapture_numpy_preprocessed)
output_dir = os.path.join(script_dir, "..", "dataset", "Totalcapture_numpy_preprocessed")
...
blaze_root = os.path.join(dataset_root, 'blazepose_numpy')
gt_root = os.path.join(dataset_root, 'gt_numpy')
...
for root, dirs, files in os.walk(blaze_root):
    for filename in files:
        if filename.endswith('.npy') and filename.startswith("blaze_"):
            ...
            # These paths will look like "blazepose_numpy/S1/acting1/..." 
